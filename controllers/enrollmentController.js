...
const EnrollmentController = {
  async create(req, res) {
    try {
      const { user_id, course_id, enrolled_at, completed_at, progress } =
        req.body;
  ...
  async getById(req, res) {
    try {
      ...
      res
        .status(500)
        .json({ message: "Failed to get enrollment", error: error.message });
    }
  },
  ...
  async getByUserId(req, res) {
    try {
      ...
      console.error(error);
      res.status(500).json({
        message: "Failed to get enrollments for user",
        error: error.message,
      });
    }
  },
