...
const AdminController = {
  ...
  async getUsers(req, res, next) {
    try {
      ...
      const { users, total } = await UserModel.getAll({
        limit,
        offset,
        search,
      });
  ...
  async getDashboardSummary(req, res) {
    try {
      ...
      const pendingCourses = parseInt(
        (await query("SELECT COUNT(*) FROM courses WHERE is_approved = false"))
          .rows[0].count,
        10
      );

      // Published courses count
      const publishedCourses = parseInt(
        (await query("SELECT COUNT(*) FROM courses WHERE is_published = true"))
          .rows[0].count,
        10
      );
      ...
      const totalEnrollments = parseInt(
        (await query("SELECT COUNT(*) FROM enrollments")).rows[0].count,
        10
      );
